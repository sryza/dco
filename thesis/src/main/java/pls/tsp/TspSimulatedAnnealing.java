...
//	private static final double MAX_TEMP = 5.0;
	...
	public Node[] runSimulatedAnnealing(Node[] nodes)
	{
		start_time = System.currentTimeMillis();
		
		Node[] best = new Node[nodes.length];
...
//		_temp = 0;
	...
	public int runStep(Node[] nodes, Node[] newNodes, long end)
	{
		int maxIter = 1;
		int newMaxIter = 1;
		for(int i=0; i<3; i++)
		...
		for (int iter = 0; iter < maxIter; iter++)
		{
			...
			{
				totalImproving++;
				LocalSearch.swap3opt(nodes, newNodes, i, j, k);
				return delta;
			}
